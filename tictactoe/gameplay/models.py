...
class Game(models.Model):
    ...
    def update_after_move(self, move):
        ...
        self.status = self._get_games_status_after_move(move)

    def _get_games_status_after_move(self, move):
        x, y = move.x, move.y
        board = self.board()
...
class Move(models.Model):
    # класс имеет три поля: x, y, primary key(первчиный ключ)
    ...
    # blank = True разрешает полю комментария оставаться пустым
    comment = models.CharField(max_length=300,
                               blank=True)

    # смысл опции CASCADE в том, что когда связанный объект удаляется, такой
    # же объект тоже.
